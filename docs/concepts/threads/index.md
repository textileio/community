...
At the core of every thread is a secret. Only peers that possess the secret are able to decrypt thread content or follow linkages.
...
- Mud puddle resistant: There should be a way to safely backup a thread's state such that it can be recovered by the owner.
- Offline-first: Because most people access the internet primarily from mobile devices, threads must enable a UX that works well in scenarios where connectivity is spotty and peers are constantly coming and going from the network.
- Secure: Updates must be signed by peers and encrypted with the shared key. Ideally, linkages should also be obscured by encryption.
...
4. [Files](/concepts/threads/files) DAG node referenced by the graph also need to be synced. This is done by first downloading the thread's schema, which contains pinning (storage) instructions for the DAG's inner files.
5. Finally, a peer may send back an acknowledgement to the author containing _its_ latest graph tip (or HEAD in git terminology) if it believes they have diverged.

## Account Sync

[Account peers](/concepts/#account-peers) are able to instruct each other to create and delete threads. This is handled by an additional signature on outbound updates, which differentiates account and non-account peer messages.
...
!!! tip
    The Blocks API can serve [Graphviz](https://www.graphviz.org/) dot files which can be converted to image as seen below. See `textile blocks --help` for more)
...
- [ ] Move to a roll-based access model for accounts and applications.
...
- [ ] In addition to being governed by access roll, peers should not be required to download thread history.
