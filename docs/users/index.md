...
User mailboxes are designed to be used with private key based identities.
...
The above assumes you are running the database in insecure environments such as the browser, but if you are running the database in a secure environment you can skip the tutorial and provide you [API keys](../hub/apis.md) directly.
...
With your database created, you now need to *start* it. Starting a database requires the data owner's identity. In these examples, we'll use a [random public-key infrastructure (PKI) based identity](../tutorials/hub/pki-identities.md), however, see [Identity](#identity) for further details and links.
